...
{
	//create the organ
	COrganInstrument *organ = new COrganInstrument(bpm);

	//set attributes of xml
	organ->SetWaveDrawbars(m_drawbars);
	organ->SetAmplitude(m_amp);
	organ->SetVibratoFreq(m_vibratoFreq);
	organ->SetVibratoFlag(m_vibratoFlag);
...
void COrganFactory::SetNote(CNote *note)
{
	...
	double m_leslieLast = m_leslieFreq;
	...
	for (int i = 0; i<len; i++)
	{
		...
		else if (name == "vibratoFlag")
		{
			value.ChangeType(VT_R8);
			m_vibratoFlag = value.dblVal;
		}
	}

	if (m_leslieFreq == m_leslieLast)
	{
...
{
	// Determine the harmonics that augment the sound produced by a key
	int digit = 0;
	double sample = 0.0;
